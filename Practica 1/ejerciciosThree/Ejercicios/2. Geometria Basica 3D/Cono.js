...
class Cono extends THREE.Object3D {
  constructor(gui,titleGui) { // referencia a la gui, y el titulo que tendra la seccion del modelo
    ...
    let geometry = new THREE.ConeGeometry(1,1,3); //radio, altura y devisiones
    // Como material se crea uno a partir de las normales
    let material = new THREE.MeshNormalMaterial();

    // Ya podemos construir el Mesh
    let modelo = new THREE.Mesh (geometry, material);
  ...
  createGUI (gui,titleGui) {
    ...
    folder.add (this.guiControls, 'resolucion', 3, 60, 1).name ('Resolucion: ').listen();
  }
  
  update () {
    //this.rotacion+=0.01;
    this.rotation.set (this.rotacion,this.rotacion,this.rotacion);
    this.scale.set (this.guiControls.radio,this.guiControls.altura,this.guiControls.resolucion);
  }
}
